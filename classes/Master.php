...
Class Master extends DBConnection {
	...
	function save_project(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = 'El nombre del proyecto ya existe.';
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = "El proyecto se ha agregado con éxito.";
				else
					$resp['msg'] = "Los detalles del proyecto se han actualizado correctamente.";
			}else{
				$resp['status'] = 'failed';
	...
	function delete_project(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['mesg'] = 'No se puede eliminar este proyecto porque ya tiene un reporte en la lista.';
		}else{
			$del = $this->conn->query("UPDATE `project_list` set delete_flag = 1 where id = '{$id}'");
			if($del){
				$resp['status'] = 'success';
				$this->settings->set_flashdata('success',"El proyecto se ha eliminado correctamente.");
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $this->conn->error;
			}
	...
	function close_project(){
		...
		if($update){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"El proyecto se ha cerrado con éxito.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_work_type(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = 'El tipo de trabajo ya existe.';
			...
			if($save){
				...
				$resp['status'] = 'success';
				if(empty($id))
					$resp['msg'] = "El tipo de trabajo se ha agregado correctamente.";
				else
					$resp['msg'] = "Los detalles del tipo de trabajo se han actualizado correctamente.";
			}else{
				$resp['status'] = 'failed';
				$resp['msg'] = "Ocurrió un error.";
				$resp['err'] = $this->conn->error."[{$sql}]";
	...
	function delete_work_type(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"El tipo de trabajo se ha eliminado correctamente.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_report(){
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = " El reporte se ha agregado con éxito.";
			else
				$resp['msg'] = " Los detalles del reporte se han actualizado correctamente.";
	...
	function delete_report(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success'," El reporte se ha eliminado correctamente.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
